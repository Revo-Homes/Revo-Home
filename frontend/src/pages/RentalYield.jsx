...
function KpiCard({ label, value, sub, accentColor }) {
  return (
    <div style={{ background: "#f9fafb", borderRadius: 12, padding: "1.2rem 1.3rem", position: "relative", overflow: "hidden" }}>
      <div style={{
        position: "absolute", top: 0, left: 0, width: 3, height: "100%",
        borderRadius: "3px 0 0 3px", background: accentColor, transition: "background 0.3s",
      }} />
      <div style={{ fontSize: 12.5, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>{label}</div>
      <div style={{ fontSize: 26, fontWeight: 500, lineHeight: 1, transition: "color 0.3s" }}>{value}</div>
      <div style={{ fontSize: 12.5, color: "#9ca3af", marginTop: 5 }}>{sub}</div>
    </div>
  );
}
...
function FormulaCard({ name, expr, result, accentColor }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12.5, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{name}</div>
      <div style={{ fontSize: 12.5, fontFamily: "monospace", color: "#9ca3af", marginBottom: 5, lineHeight: 1.5 }}>{expr}</div>
      <div style={{ fontSize: 15, fontWeight: 500, color: "#111827" }}>{result}</div>
    </div>
  );
}
...
function ReturnBar({ label, pct, color, value }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
      <span style={{ fontSize: 12.5, color: "#6b7280", width: 80, flexShrink: 0, textAlign: "right" }}>{label}</span>
      <div style={{ flex: 1, height: 6, background: "#e5e7eb", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ height: "100%", borderRadius: 3, background: color, width: `${pct}%`, transition: "width 0.4s, background 0.4s" }} />
      </div>
      <span style={{ fontSize: 12.5, fontWeight: 500, width: 40, textAlign: "right", color: "#374151" }}>{value}</span>
    </div>
  );
}
...
export default function RentalYieldCalculator() {
  ...
  margin: "0 auto",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem", gap: 12, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 600, margin: 0, lineHeight: 1.2 }}>Rental yield calculator</h1>
          <p style={{ fontSize: 13, color: "#6b7280", margin: "2px 0 0" }}>Real estate ROI analysis · India</p>
        </div>
        <div style={{
  display: "inline-flex",
  ...
  fontSize: 14,
  fontWeight: 700,
  background: `linear-gradient(135deg, ${verdictTier.bg}, #ffffff)`,
  color: verdictTier.text,
  border: `2px solid ${verdictTier.border}`,
  boxShadow: `0 8px 20px ${verdictTier.border}`,
  position: "relative",
  overflow: "hidden"
}}>
  <span style={{
    ...
    height: 9,
    borderRadius: "50%",
    background: verdictTier.color,
    animation: "ryPulse 1.2s infinite"
  }} />
  ...
  <span style={{
    position: "absolute",
    inset: 0,
    background: `radial-gradient(circle at top left, ${verdictTier.color}22, transparent 60%)`
  }} />
</div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.2rem", marginBottom: "1.25rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1.2rem" }}>
          <div style={{ fontSize: 12.5, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: "1.1rem" }}>
            Your property
          </div>
          ...
          {/* Return breakdown bars */}
          <div style={{ fontSize: 12.5, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>
            Return breakdown
          </div>
          <ReturnBar label="Net yield"    pct={netBarPct}   color={tier.color} value={netYield > 0 ? netYield.toFixed(2) + "%" : "—"} />
          <ReturnBar label="Appreciation" pct={87}          color="#1D9E75"    value="~7%" />
        ...
        <div style={{ display: "flex", flexDirection: "column",gap: "1.2rem"  }}>
            ...
            {TIERS.map((t) => {
              ...
              return (
                <div key={t.key} style={{
                  ...
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: t.color, flexShrink: 0 }} />
                  <span style={{ fontSize: 12.5, color: "#6b7280", width: 48, flexShrink: 0 }}>{rangeLabel}</span>
                  <span style={{ fontSize: 12.5, fontWeight: 500, flex: 1, color: active ? t.text : "#374151" }}>{t.label}</span>
                  {/* Badge */}
                  <span style={{
                    fontSize: 10, fontWeight: 500, padding: "2px 8px", borderRadius: 99,
                    background: t.badge.bg, color: t.badge.text,
          ...
          {/* Investment quality */}
          <div style={{ background: "#fff", border: "0.5px solid #e5e7eb", borderRadius: 14, padding: "1.25rem", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
            <div style={{ fontSize: 12.5, fontWeight: 500, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: "1.1rem" }}>
              Investment quality
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              {/* Risk */}
              <div>
                <div style={{ fontSize: 12.5, color: "#6b7280", marginBottom: 6 }}>Risk level</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, height: 5, background: "#e5e7eb", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{
                      height: "100%", borderRadius: 3,
              ...
              {/* Stars */}
              <div>
                <div style={{ fontSize: 12.5, color: "#6b7280", marginBottom: 6 }}>Market rating</div>
                <div style={{ display: "flex", gap: 3 }}>
                  {[1,2,3,4,5].map((i) => (
                    <span key={i} style={{
                      fontSize: 16, transition: "color 0.3s",
